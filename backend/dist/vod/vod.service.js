...
const link_tester_service_1 = require("../tv/link-tester.service");
let VodService = VodService_1 = class VodService {
    constructor(prisma, linkTesterService) {
        this.prisma = prisma;
        this.linkTesterService = linkTesterService;
        this.logger = new common_1.Logger(VodService_1.name);
    }
    ...
    async getMovies(filters) {
        ...
        if (filters?.hasErrors) {
            result = result.filter((m) => m.sources.some((s) => s.last_status === 'ERROR' || s.last_status === 'UNAVAILABLE'));
        }
    ...
    async addMovieSource(dto, userId) {
        ...
        await this.testMovieSource(source.id, source.url, source.format);
    ...
    async updateMovieSource(id, dto) {
        ...
        if (!source)
            throw new common_1.NotFoundException('Fuente de película no encontrada.');
    ...
    async testMovieSource(sourceId, url, format) {
        ...
        const result = await this.linkTesterService.testLink(targetUrl, format || source?.format);
        if (source) {
            await this.prisma.movieSource.update({
                where: { id: sourceId },
                data: {
    ...
    async addEpisode(dto) {
        ...
        if (!season)
            throw new common_1.BadRequestException('Temporada no encontrada.');
    ...
    async updateEpisode(id, dto) {
        ...
        if (!episode)
            throw new common_1.NotFoundException('Episodio no encontrado.');
    ...
    async addEpisodeSource(dto, userId) {
        const format = dto.format || this.linkTesterService.detectFormat(dto.url);
        const priority = dto.priority ?? 1;
        if (dto.is_active !== false && priority === 1) {
        ...
        await this.testEpisodeSource(source.id, source.url, source.format);
        const episode = await this.prisma.episode.findUnique({
    ...
    async updateEpisodeSource(id, dto) {
        ...
        if (!source)
            throw new common_1.NotFoundException('Fuente de episodio no encontrada.');
    ...
    async testEpisodeSource(sourceId, url, format) {
...
exports.VodService = VodService = VodService_1 = __decorate([
