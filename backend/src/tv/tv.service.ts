...
export class TvService {
  ...
  /**
   * Seed default categories and demo channels if empty
   */
  ...
  async createCategory(dto: CreateCategoryDto) {
    ...
    if (existing) {
      throw new BadRequestException(`Ya existe una categoría con el identificador / slug: "${slug}"`);
    }
  ...
  async updateCategory(id: string, dto: UpdateCategoryDto) {
    ...
    if (slug !== category.slug) {
      ...
      if (existing && existing.id !== id) {
        throw new BadRequestException(`El slug "${slug}" ya está en uso por otra categoría.`);
      }
    }
  ...
  async getChannels(filters?: {
    ...
    let result = channels.map((ch) => {
      ...
      // Active source with highest priority (lowest numerical priority value)
    ...
    if (filters?.hasErrors) {
      result = result.filter((ch) =>
        ch.sources.some((s) => s.last_status === 'ERROR' || s.last_status === 'UNAVAILABLE'),
      );
    }
  ...
  async createChannel(dto: CreateChannelDto) {
    ...
    const initialUrl = dto.initial_source_url || (dto as any).stream_url;
  ...
  /**
   * CHANNEL SOURCES & PRIORITIES
   */
  async addSource(dto: CreateChannelSourceDto, userId?: string) {
    const channel = await this.prisma.channel.findUnique({ where: { id: dto.channel_id } });
    if (!channel) throw new BadRequestException('El canal especificado no existe.');

    const format = dto.format || this.linkTesterService.detectFormat(dto.url);
    const priority = dto.priority ?? 1;

    // If setting priority 1 and active, adjust other sources' priorities
    ...
    // Test the newly created link automatically
    await this.testAndStoreResult(source.id, source.url, source.format as StreamFormat);

    return this.getChannelById(dto.channel_id);
  }

  async updateSource(id: string, dto: UpdateChannelSourceDto) {
    const source = await this.prisma.channelSource.findUnique({ where: { id } });
    if (!source) throw new NotFoundException('Fuente de reproducción no encontrada.');
  ...
  /**
   * Quick Switch Source (Cambio Rápido de Fuente)
   */
  async quickSwitchSource(dto: QuickSwitchSourceDto, userId?: string) {
    const channel = await this.prisma.channel.findUnique({ where: { id: dto.channel_id } });
    if (!channel) throw new BadRequestException('El canal no existe.');

    const format = dto.format || this.linkTesterService.detectFormat(dto.url);

    // Demote existing priority 1 active sources
    await this.demoteExistingPriorityOne(dto.channel_id);

    // Create or promote new source
    ...
    // Test link and return updated channel
    await this.testAndStoreResult(newSource.id, newSource.url, format as StreamFormat);

    return this.getChannelById(dto.channel_id);
  }

  private async demoteExistingPriorityOne(channelId: string, excludeSourceId?: string) {
  ...
  /**
   * LINK TESTING (▶ Probar enlace)
   */
  async testLink(dto: TestLinkDto): Promise<StreamTestResultDto> {
    const result = await this.linkTesterService.testLink(dto.url, dto.format as StreamFormat);
  ...
  private async testAndStoreResult(sourceId: string, url: string, format: StreamFormat) {
   ...
   */
  async getStats(): Promise<TvStatsSummaryDto> {
    await this.ensureSeedData();
    ...
    const channelsWithoutSource = channels.filter(
      (c) => !c.sources.some((s) => s.is_active),
    ).length;
    ...
    const errorSources = allSources.filter(
      (s) => s.last_status === 'ERROR' || s.last_status === 'UNAVAILABLE',
    ).length;
        ...
        return {
          ...
          active_source: topSource
            ? {
                id: topSource.id,
                url: topSource.url,
                format: topSource.format as StreamFormat,
